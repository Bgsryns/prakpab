...
class TransaksiController extends Controller
{
    ...
    public function produk($id)
    {
        ...
        try {
            ...
            $response = $client->post('https://api.rajaongkir.com/starter/cost', [
                'headers' => [
                    'key' => env('RAJAONGKIR_API_KEY'), // API Key
    ...
    public function hapus_keranjang($id) // Menambahkan parameter $id
    {
        $transaksi = Transaksi::find($id); // Mendapatkan transaksi berdasarkan ID
        if ($transaksi) { // Memastikan transaksi ada
            $transaksi->delete();
            return redirect()->route('transaksi.keranjang')->with('success', 'Item Berhasil Dihapus');
        }
        return redirect()->route('transaksi.keranjang')->with('error', 'Item tidak ditemukan'); // Menangani jika transaksi tidak ada
    }
}